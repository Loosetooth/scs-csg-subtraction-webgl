...
export class SCSRenderer {
  ...
  /**
   * Use the SCS algorithm to render the CSG intersection
   * of the `primitive` and the primitives in `sequence`,
   * using the OpenCSG approach of transferring primitive surface
   * visibility using an alpha-channel coded ID.
   *
   * @param scene
   ...
   */
   renderIntersect(
    ...
    const render = () => {
      ...
      // For each intersecting primitive P
      for (let i = 0; i < sequence.length; i++) {
        stencilRef++;
        id++;
        ...
        // Flag in stencil fragments where Z_F_Back > Z
        this.state.set({
          colorMask: [false, false, false, false],
          // colorMask: [true, true, true, true],
          depthTest: true,
          ...
          stencilTest: true,
          stencilMask: ~0,
          stencilFunc: [gl.ALWAYS, 1, ~0],
          stencilOp: [gl.ZERO, gl.ZERO, gl.REPLACE],
          faceCulling: true,
          cullFace: gl.FRONT
        });
        drawMeshDirect(this.renderer, p, camera);

        // If Z_F_Front > Z, Z = Z_F_Front
        this.state.set({
          // colorMask: [false, false, false, false],
          colorMask: [true, true, true, true],
          faceCulling: true,
          ...
          stencilMask: ~0,
          stencilFunc: [gl.ALWAYS, stencilRef, ~0],
          stencilOp: [gl.ZERO, gl.REPLACE, gl.REPLACE],
          depthMask: true,
          depthFunc: gl.GREATER
        });
        drawMeshDirect(this.renderer, p, camera, alphaIDMaterial(id));

        // If Z_F_Back < Z, alpha = 0
        this.state.set({
          // colorMask: [false, false, false, false],
          colorMask: [true, true, true, true],
          faceCulling: true,
        ...
        drawMeshDirect(this.renderer, p, camera, alphaIDMaterial(0));
        ...
        this.state.set({
          ...
          faceCulling: true,
          cullFace: gl.FRONT,
          stencilFunc: [gl.NOTEQUAL, stencilRef, ~0],
          stencilOp: [gl.ZERO, gl.ZERO, gl.ZERO],
          depthMask: false,
          depthFunc: gl.ALWAYS
        });
      ...
      // // For each fragment F_Front of X
      // // If stencil === 1, draw F_Front
      // this.state.set({
      //   // colorMask: [false, false, false, false],
      //   colorMask: [true, true, true, true],
      //   depthMask: false,
    ...
    const render2 = i => () => {
      ...
      // Draw back of X where alphaID = 1
      this.state.set({
        cullFace: gl.FRONT,
        depthMask: true,
        depthFunc: gl.LESS,
